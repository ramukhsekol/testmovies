...
public class JSONServiceImpl implements JSONService {
  ...
  public List<Movie> getMovies(int sindex) {
    try {
      ...
      Document doc = Jsoup.connect("https://3movierulz.vc/category/telugu-movie/page/" + sindex)
      ...
      for (Element element : elements) {
        if (movieindex > 2) {
          ...
          Document document =
              Jsoup.connect(linkHref).timeout(10000).validateTLSCertificates(false).get();
          ...
          for (Element para : paragraphs) {
            if (i == 1) {
              ...
              if (!movieName.trim().contains("Thirunaal")
                  && !movieName.trim().contains("Seven")
                ...
                Integer movieYear = Integer.parseInt(
                    para.text().substring(para.text().indexOf("(") + 1, para.text().indexOf(")")));
  ...
  public boolean generateJsonFile(int index, List<Movie> movies) {
    File file = uploadPathService.getFilePath(index + ".json", "movies/telugu");
  ...
  public List<Alphamovies> getAplhaMovies(int sindex) {
    try {
      ...
      Document doc = Jsoup.connect("https://ww.0123movies.su/library/B/page/" + sindex + "/")
          .timeout(10000).validateTLSCertificates(false).get();
      Element body = doc.body();
      ...
      for (Element element : elements) {
        ...
        Document cdocument =
            Jsoup.connect(plinkHref).timeout(10000).validateTLSCertificates(false).get();
  ...
  public boolean generateAlphaJsonFile(int index, List<Alphamovies> movies) {
    File file = uploadPathService.getFilePath(index + ".json", "alpha/b");
    ObjectMapper objectMapper = new ObjectMapper();
    try {
      objectMapper.writeValue(file, movies);
      return true;
  ...
  public List<Alphamovies> getMoviesByIndex(String movietype, String pageIndex) {
    File file =
        uploadPathService.getFilePath(pageIndex + ".json", "alpha/" + movietype.toLowerCase());
    ObjectMapper objectMapper = new ObjectMapper();
    try {
